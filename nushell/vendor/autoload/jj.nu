module completions {
  ...
  def "nu-complete jj debug init-local color" [] {
    [ "always" "never" "debug" "auto" ]
  }

  # Create a new repo in the given directory using the proof-of-concept native backend
  export extern "jj debug init-local" [
    ...
    --at-op: string           # Operation to load the repo at
    --debug                   # Enable debug logging
    --color: string@"nu-complete jj debug init-local color" # When to colorize output
    --quiet                   # Silence non-primary command output
    --no-pager                # Disable the pager
    --config: string          # Additional configuration options (can be repeated)
    --config-toml: string     # Additional configuration options (can be repeated) (DEPRECATED)
